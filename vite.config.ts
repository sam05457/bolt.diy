...
const getGitInfo = () => {
  try {
    return {
      ...
      commitTime: execSync('git log -1 --format=%cd', { stdio: 'pipe' }).toString().trim(),
      author: execSync('git log -1 --format=%an', { stdio: 'pipe' }).toString().trim(),
      email: execSync('git log -1 --format=%ae', { stdio: 'pipe' }).toString().trim(),
      remoteUrl: execSync('git config --get remote.origin.url', { stdio: 'pipe' }).toString().trim(),
      repoName: execSync('git config --get remote.origin.url', { stdio: 'pipe' })
        .toString()
        .trim()
        .replace(/^.*github.com[:/]/, '')
        .replace(/\.git$/, ''),
